...
// Clear Cache Engine
// ==========================================
function promptClearAllCache() {
  const modal = document.getElementById("clear-cache-modal");
  if (modal) modal.classList.add("active");
}

function executeClearAllCache() {
  closeModal("clear-cache-modal");
  
  // Clear all kdp localStorage keys
  const keysToRemove = [];
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i);
    if (k && (k.startsWith("kdp_") || k.includes("project"))) {
      keysToRemove.push(k);
    }
  }
  ...
  fetchRecentProjects();
  calculateKdpRoyalty();
  showToast("🧹 All project caches & temporary buffers cleared successfully!", "success");
}

// ==========================================
// Amazon KDP Pricing & Royalty Calculator Engine
// ==========================================
function calculateKdpRoyalty() {
  const mkt = document.getElementById("est-marketplace")?.value || "US";
  const interior = document.getElementById("est-interior")?.value || "bw";
  const pagesSlider = document.getElementById("est-pages-slider");
  const pages = pagesSlider ? parseInt(pagesSlider.value) || 40 : 40;
  const listPriceInput = document.getElementById("est-list-price");
  const listPrice = listPriceInput ? parseFloat(listPriceInput.value) || 7.99 : 7.99;

  const currSymbolEl = document.getElementById("est-curr-symbol");
  let curr = "$";
  if (mkt === "UK") curr = "£";
  else if (mkt === "EU") curr = "€";
  else if (mkt === "CA") curr = "CA$";
  else if (mkt === "AU") curr = "AU$";

  if (currSymbolEl) currSymbolEl.innerText = curr;

  // Calculate Printing Cost per Official Amazon KDP Formula
  let printCost = 0.0;
  if (mkt === "US") {
    if (interior === "bw") {
      printCost = pages <= 108 ? 2.30 : (1.00 + (pages * 0.012));
    } else if (interior === "color_std") {
      printCost = 1.00 + (pages * 0.036);
    } else { // Premium Color
      printCost = pages <= 40 ? 3.65 : (1.00 + (pages * 0.070));
    }
  } else if (mkt === "UK") {
    if (interior === "bw") {
      printCost = pages <= 108 ? 1.93 : (0.80 + (pages * 0.0105));
    } else if (interior === "color_std") {
      printCost = 0.80 + (pages * 0.031);
    } else {
      printCost = pages <= 40 ? 3.00 : (0.80 + (pages * 0.060));
    }
  } else if (mkt === "EU") {
    if (interior === "bw") {
      printCost = pages <= 108 ? 2.05 : (0.85 + (pages * 0.012));
    } else if (interior === "color_std") {
      printCost = 0.85 + (pages * 0.035);
    } else {
      printCost = pages <= 40 ? 3.25 : (0.85 + (pages * 0.065));
    }
  } else if (mkt === "CA") {
    if (interior === "bw") {
      printCost = pages <= 108 ? 3.10 : (1.35 + (pages * 0.016));
    } else {
      printCost = 1.35 + (pages * 0.048);
    }
  } else if (mkt === "AU") {
    if (interior === "bw") {
      printCost = pages <= 108 ? 3.50 : (1.50 + (pages * 0.018));
    } else {
      printCost = 1.50 + (pages * 0.055);
    }
  }

  // Minimum List Price = Print Cost / 0.60
  const minListPrice = printCost / 0.60;
  // Author Royalty = (List Price * 0.60) - Print Cost
  const royalty = Math.max(0, (listPrice * 0.60) - printCost);
  const profitMargin = listPrice > 0 ? ((royalty / listPrice) * 100.0) : 0.0;

  // Update UI Elements
  const minHint = document.getElementById("est-min-price-hint");
  if (minHint) minHint.innerText = `Min: ${curr}${minListPrice.toFixed(2)}`;

  const printCostEl = document.getElementById("est-res-print-cost");
  if (printCostEl) printCostEl.innerText = `${curr}${printCost.toFixed(2)}`;

  const royaltyEl = document.getElementById("est-res-royalty");
  if (royaltyEl) royaltyEl.innerText = `${curr}${royalty.toFixed(2)}`;

  const marginEl = document.getElementById("est-res-margin");
  if (marginEl) marginEl.innerText = `${profitMargin.toFixed(1)}%`;

  const minPriceEl = document.getElementById("est-res-min-price");
  if (minPriceEl) minPriceEl.innerText = `${curr}${minListPrice.toFixed(2)}`;

  // Monthly Projected Tiers
  const t50 = document.getElementById("est-tier-50");
  const t200 = document.getElementById("est-tier-200");
  const t500 = document.getElementById("est-tier-500");
  const t1000 = document.getElementById("est-tier-1000");

  if (t50) t50.innerText = `${curr}${(royalty * 50).toFixed(2)}`;
  if (t200) t200.innerText = `${curr}${(royalty * 200).toFixed(2)}`;
  if (t500) t500.innerText = `${curr}${(royalty * 500).toFixed(2)}`;
  if (t1000) t1000.innerText = `${curr}${(royalty * 1000).toFixed(2)}`;
}

function onEstPageSlider(val) {
  const countVal = document.getElementById("est-page-count-val");
  if (countVal) countVal.innerText = `${val} Pages`;
  calculateKdpRoyalty();
}

function setEstPrice(price) {
  const input = document.getElementById("est-list-price");
  if (input) input.value = price.toFixed(2);
  calculateKdpRoyalty();
}

function syncEstimatorWithCurrentBook() {
  const contentPages = (currentProject.pages || []).filter(p => p.page_type !== "blank_verso" && !p.page_type?.startsWith("front_matter_"));
  const bType = currentProject.book_type || "coloring_book";
  const isColoring = (bType === "coloring_book");
  
  // Total compiled pages (front matter + content + blank backs)
  const totalCompiled = isColoring ? (4 + (contentPages.length * 2)) : (1 + contentPages.length);
  const targetPages = Math.max(24, totalCompiled);

  const slider = document.getElementById("est-pages-slider");
  if (slider) {
    slider.value = targetPages;
    onEstPageSlider(targetPages);
  }

  showToast(`⚡ Synced Estimator with "${currentProject.name || 'Book'}" (${targetPages} compiled pages)!`, "success");
}
